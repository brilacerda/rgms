...
Feature: orientations
  ...
  Scenario: list existing orientation
    Given the system has orientation entitled "The Book is on the table"
    When I view the orientation list
    Then my orientation list contains "The Book is on the table"

  Scenario: order by existing orientation by type
  ...
  Scenario: order by existing orientation by type web
    Given I am at the orientation page
    And I create some orientation with type of "Mestrado"
    When I select to view the list of orientation column name "Tipo"
    Then my orientation list shows only the orientation type of by "Mestrado"

  Scenario: order by existing orientation by leader
    Given the system has some orientation supervised by "Tomaz"
    When the system filter the articles supervised by leader "Tomaz"
    Then the system orientation list content is not modified

  Scenario: order by existing orientation by leader web
    Given I am at the orientation page
    And I create some orientation supervised by "Tomaz"
    When I select to view the list of orientation column name "Orientando"
    Then my orientation list shows only the orientation supervised by "Tomaz"